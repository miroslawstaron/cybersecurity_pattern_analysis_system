import pytest
from cylberta_embeddings import extract_embeddings_cylberta, calculate_distances_cylberta, analyze_cylberta
import pandas as pd
import os
...
def embeddings_df():
  ...
  data = {
    'code1.c': [0.1, 0.2, 0.3, 0.4, 0],
    'code2.c': [0.5, 0.6, 0.7, 0.8, 0],
...
def test_extract_embeddings_cylberta():
  ...
  df = extract_embeddings_cylberta(TEST_FOLDER, TEST_CODE_FOLDER, TEST_REFERENCE_FOLDER)
  assert isinstance(df, pd.DataFrame)
  # check if the dataframe has the expected shape and columns
  assert df.shape == (4, 5) # 4 files, 5 features
  assert list(df.columns) == ['e1', 'e2', 'e3', 'e4', 'Modified']
  # check if the dataframe has the expected values for the Modified column
  assert df.loc[df.index.str.contains('VCE_'), 'Modified'].values[0] == 2
  assert df.loc[df.index.str.contains('SCE_'), 'Modified'].values[0] == 1
  assert df.loc[df.index.str.contains('code'), 'Modified'].values[0] == 0
...
def test_calculate_distances_cylberta(embeddings_df):
  ...
  # check if the function returns a dataframe
  df = calculate_distances_cylberta(TEST_FOLDER)
  assert isinstance(df, pd.DataFrame)
  # check if the dataframe has the expected shape and columns
  ...
  assert df.loc[(df['Reference'] == 'SCE_1.c') & (df['Module'] == 'code2.c'), 'Distance'].values[0] > 0.2
  assert df.loc[(df['Reference'] == 'VCE_1.c') & (df['Module'] == 'code1.c'), 'Distance'].values[0] > 0.2
...
def test_analyze_cylberta(distances_df, capsys):
  ...
  assert analyze_cylberta(TEST_FOLDER) == 1
  ...
  assert 'Module code2.c is flagged as secure' in captured.out
